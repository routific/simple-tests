          ...
          <div className="bg-background">
            {releaseRuns.length === 0 ? (
              ...
              <div className="divide-y divide-border">
                {releaseRuns.map((run) => (
                  <RunRow key={run.id} run={run} linearWorkspace={linearWorkspace} onDuplicate={() => handleOpenDuplicate(run)} onDelete={() => handleDelete(run)} />
                ))}
              </div>
            )}
          </div>
...
function RunRow({ run, linearWorkspace, onDuplicate, onDelete }: { run: RunWithStats; linearWorkspace?: string; onDuplicate: () => void; onDelete: () => void }) {
